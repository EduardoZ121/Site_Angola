...
| **Tipo**                     | BLOCKER de _âmbito_ (não de código)                                                                        |
...
A autorização de _qualquer_ correção de código continua a ser uma frase explícita do Founder (escopo + fase). Esta baseline **não** contém `AUTORIZO: FASE 1`.
